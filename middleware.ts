...
export const config = {
  // run on everything except static assets & api you want to keep alive
  matcher: ["/((?!_next|favicon.ico|api/stripe-webhook).*)"],
};

export function middleware(req: NextRequest) {
  // Toggle maintenance with an env var (set to "1" on Vercel to enable)
  const locked = process.env.SITE_LOCK === "1";

  if (!locked) return NextResponse.next();

  // Optional: allow yourself in with HTTP Basic Auth (set BASIC_USER / BASIC_PASS)
  ...
  if (user && pass) {
    ...
    if (auth?.startsWith("Basic ")) {
      ...
      if (u === user && p === pass) {
        return NextResponse.next(); // let you through
      }
    }
  ...
  // No password set → show friendly maintenance HTML
...
<html lang="en">
  <head>
    ...
    <style>
      ...
      body{margin:0; display:grid; place-items:center; background:#fff; color:var(--text); font:16px/1.45 system-ui, -apple-system, Segoe UI, Roboto, Inter, Arial, sans-serif;}
      ...
      p{margin:6px 0}
      .pill{display:inline-block; padding:6px 10px; border-radius:999px; background:var(--pink); font-weight:700; margin-bottom:8px}
    </style>
  </head>
  <body>
    <div class="card">
